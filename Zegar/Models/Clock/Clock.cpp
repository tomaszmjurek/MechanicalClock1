...
#include "Models/Clock/ClockInternal.h"

Clock::Clock(Shader* shader, Texture* tex, glm::vec3 pos) {
	setName(ClockInternal::name);
	setVertices(ClockInternal::vertices);
	setVertexNormals(ClockInternal::vertexNormals);
	setTexCoords(ClockInternal::texCoords);
	setVertexCount(ClockInternal::vertexCount);
	setShader(shader);
	setPostiotion(pos);
	texture = tex;
	prepareObject();
...
void Clock::drawObject(glm::mat4 mP, glm::mat4 mV) {
	//Wylicz macierz modelu rysowanego obiektu

	glm::mat4 mM = glm::mat4(1.0f);
	mM = glm::translate(mM, getPosition());
	glm::mat4 rotateY = glm::rotate(mM, 3.14f * 180 / 180, glm::vec3(0, 1, 0));
	mM = mM * rotateY;
