...
#include "Models/Chair/ChairInternal.h"

Chair::Chair(Shader* shader, Texture* tex, glm::vec3 pos) {
	setName(ChairInternal::name);
	setVertices(ChairInternal::vertices);
	setVertexNormals(ChairInternal::vertexNormals);
	setTexCoords(ChairInternal::texCoords);
	setVertexCount(ChairInternal::vertexCount);
	setShader(shader);
	setPostiotion(pos);
	texture = tex;
	prepareObject();
...
void Chair::drawObject(glm::mat4 mP, glm::mat4 mV) {
