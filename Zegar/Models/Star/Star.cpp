...
#include "Models/Star/StarInternal.h"

Star::Star(Shader* shader, Shader* lShader, Texture* tex, glm::vec3 pos) {
	setName(StarInternal::name);
	setVertices(StarInternal::vertices);
	setVertexNormals(StarInternal::vertexNormals);
	setTexCoords(StarInternal::texCoords);
	setVertexCount(StarInternal::vertexCount);
	setShader(shader);
	setPostiotion(pos);
	texture = tex;
	lightShader = lShader;
...
void Star::drawObject(glm::mat4 mP, glm::mat4 mV) {
...
std::vector <glm::vec3> Star::lightPosition() {
