...
#include "Models/MinIndicator/MinIndicatorInternal.h"

MinIndicator::MinIndicator(Shader* shader, Texture* tex, glm::vec3 pos, float deg) {
	setName(MinIndicatorInternal::name);
	setVertices(MinIndicatorInternal::vertices);
	setVertexNormals(MinIndicatorInternal::vertexNormals);
	setTexCoords(MinIndicatorInternal::texCoords);
	setVertexCount(MinIndicatorInternal::vertexCount);
	setShader(shader);
	setPostiotion(pos);
	texture = tex;
	angle = deg;
...
void MinIndicator::drawObject(glm::mat4 mP, glm::mat4 mV) {
...
void MinIndicator::prepareObject() {
