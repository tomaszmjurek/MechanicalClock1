...
#include "Models/Wardrobe/WardrobeInternal.h"

Wardrobe::Wardrobe(Shader* shader, Texture* tex, glm::vec3 pos) {
	setName(WardrobeInternal::name);
	setVertices(WardrobeInternal::vertices);
	setVertexNormals(WardrobeInternal::vertexNormals);
	setTexCoords(WardrobeInternal::texCoords);
	setVertexCount(WardrobeInternal::vertexCount);
	setShader(shader);
	setPostiotion(pos);
	texture = tex;
	prepareObject();
...
void Wardrobe::drawObject(glm::mat4 mP, glm::mat4 mV) {
